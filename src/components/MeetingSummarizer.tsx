...
export function MeetingSummarizer() {
  ...
  useEffect(() => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      ...
      setError(
        "Audio recording is not supported in this browser. Try using Chrome or Firefox."
      );
    }
    const checkApiKeys = async () => {
      ...
      if (!geminiKey) {
        setError(
          "Google Gemini API key is missing. Please check your .env file."
        );
      } else if (!huggingfaceKey) {
        setError(
          "Hugging Face API key is missing. Please add it to the .env file."
        );
      }
    };
    checkApiKeys();
      ...
      mediaRecorder.onstop = () => {
        try {
          ...
          if (audioBlob.size < 1000) {
            throw new Error(
              "Audio recording is too short or empty. Please try again."
            );
          }
  ...
  const stopRecording = async () => {
    ...
    if (audioChunksRef.current.length > 0) {
      ...
      try {
        ...
        if (audioBlob.size < 1000) {
          throw new Error(
            'Audio recording is too short or empty. Please try again or use the "Test with Sample Text" option.'
          );
        }
        const formData = new FormData();
        formData.append("audio", audioBlob, "meeting.webm");
        const response = await fetch(
          "https://speech2text-6n0t.onrender.com/api/speech2text",
          {
            method: "POST",
            body: formData,
          }
        ...
        if (!response.ok) {
          throw new Error(
            "Failed to get transcription and summary from the server."
          );
        }
      ...
      setError(
        'No audio data was recorded. Please try again or use the "Test with Sample Text" option.'
      );
    }
  };
  ...
  const testWithSampleText = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to process sample text"
      );
    } finally {
      setTranscribing(false);
      setLoading(false);
  ...
  const extractMeetingDetails = (input: string) => {
    ...
    try {
      ...
      const dateTimeLine = input
        .split("\n")
        .find((line) =>
          /[A-Za-z]+,\s+[A-Za-z]+\s+\d{1,2}\s*·\s*[0-9]{1,2}:[0-9]{2}\s*[–-]\s*[0-9]{1,2}:[0-9]{2}(am|pm)?/i.test(
            line
          )
        );
      if (dateTimeLine) {
        ...
        const timeRegex =
          /([0-9]{1,2}:[0-9]{2})\s*[–-]\s*([0-9]{1,2}:[0-9]{2})(am|pm)?/i;
  ...
  const groupedMeetings = filteredCalendarMeetings.reduce(
    (acc: Record<string, MeetingSummary[]>, meeting) => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-inter">
      <aside
        className={`w-72 bg-white border-r flex flex-col py-6 px-5 fixed top-0 left-0 h-100 z-40 
        ...
        <nav className="flex-1">
          <ul className="space-y-2">
            <li>
              <Link
                to="/"
                className="flex items-center text-gray-700 hover:text-blue-700 font-medium rounded px-3 py-2 bg-blue-50"
              >
            ...
            <li>
              <Link
                to="/history"
                className="flex items-center text-gray-700 hover:text-blue-700 font-medium rounded px-3 py-2"
              >
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between px-4 md:px-10 py-6 bg-white border-b">
          ...
          <div className="flex items-center space-x-6 mt-4 md:mt-0">
            <nav className="flex space-x-6 text-gray-600 font-medium">
              <span className="border-b-2 border-transparent hover:border-blue-600 cursor-pointer">
                Meetings
              </span>
          ...
          <section className="flex-1 px-4 md:px-10 py-8 overflow-y-auto">
            ...
            <div className="mb-8 p-4 bg-white border border-gray-200 rounded-lg">
              <div className="font-semibold mb-2 text-gray-700">
                Extract Meeting Details from Link
              </div>
              ...
              {meetingDetails.platform && (
                <div className="mt-4 text-gray-800">
                  <div>
                    <strong>Platform:</strong> {meetingDetails.platform}
                  </div>
                  {meetingDetails.platform === "Zoom" && (
                    <div className="my-2">
                      <img
                        src="https://portal-media.cca.edu/images/Zoom_banner2x.width-1130.png"
                        alt="Zoom"
                        style={{ height: 40 }}
                      />
                    </div>
                  ...
                  {meetingDetails.platform === "Google Meet" && (
                    <div className="my-2">
                      <img
                        src="https://community.pepperdine.edu/it/images/google-meet-1440x430.jpg"
                        alt="Google Meet"
                        style={{ height: 40 }}
                      />
                    </div>
                  ...
                  {meetingDetails.platform === "Microsoft Teams" && (
                    <div className="my-2">
                      <img
                        src="https://upload.wikimedia.org/wikipedia/commons/4/4e/Microsoft_Office_Teams_%282018%E2%80%93present%29.svg"
                        alt="Teams"
                        style={{ height: 40 }}
                      />
                    </div>
                  ...
                  {meetingDetails.duration && (
                    <div>
                      <strong>Duration:</strong> {meetingDetails.duration}
                    </div>
                  )}
                  {meetingDetails.timezone && (
                    <div>
                      <strong>Time Zone:</strong> {meetingDetails.timezone}
                    </div>
                  )}
                </div>
              )}
            ...
            {Object.keys(groupedMeetings).length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full">
                ...
                <div className="mb-4 text-sm text-gray-500 text-center">
                  Looks like you don't have any events on your calendar for the
                  next 7 days.
                  <br />
                  You can create new events in your calendar app and then
              ...
              Object.entries(groupedMeetings).map(([date, meetings]) => (
                <div key={date} className="mb-10">
                  ...
                  {meetings.map((meeting) => (
                    <div
                      key={meeting.id}
                      className="flex items-center bg-white border border-gray-200 rounded-xl px-6 py-5 mb-6 shadow-sm"
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500 mb-2">
                          {meeting.date
                            ...
                            : ""}{" "}
                          &middot; {Math.round(meeting.duration / 60)} min
                          &middot; {userEmail}
                        </div>
                        {meeting.transcript && (
